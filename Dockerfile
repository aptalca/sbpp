...
RUN \
 ...
 mkdir -p \
	/var/www/html/ && \
 if [ -z ${SBPP_RELEASE+x} ]; then \
	SBPP_RELEASE=$(curl -sX GET "https://api.github.com/repos/sbpp/sbpp/releases/latest" \
	| awk '/tag_name/{print $4;exit}' FS='[""]'); \
 fi && \
 curl -o \
 /tmp/sbppit.tar.gz -L \
	"https://github.com/sbpp/sbpp/archive/${SBPP_RELEASE}.tar.gz" && \
 tar xf \
 /tmp/sbppit.tar.gz -C \
